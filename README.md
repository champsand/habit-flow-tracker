...
Before zipping or sharing the source code, do not include:
...
PowerShell cleanup commands for generated files:
...
Remove-Item -Recurse -Force backend\coverage -ErrorAction SilentlyContinue
...
For GitHub, do not commit `.env`, `.env.local`, generated folders, logs, or old zip files. Keep `.env.example` files only.

After cleanup, restore and run the project with `npm install`, `npm run prisma:generate`, and the normal dev commands in each app folder.
...
Source folders such as `backend/src`, `backend/prisma`, `frontend/app`, `frontend/components`, and `frontend/lib` should remain trackable.
